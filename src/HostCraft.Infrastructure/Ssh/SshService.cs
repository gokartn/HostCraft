...
public class SshService : ISshService
{
    ...
    {
        // Localhost doesn't need SSH
        if (IsLocalhostServer(server))
        {
            _logger.LogDebug("Skipping SSH validation for localhost server {ServerName}", server.Name);
            return true;
        }
    ...
    {
        // Localhost doesn't need SSH - services should use local Docker client instead
        if (IsLocalhostServer(server))
        {
            _logger.LogWarning("SSH execution requested for localhost server {ServerName} - this should not happen. Services should use local Docker client.", server.Name);
            return new SshCommandResult(-1, string.Empty, "SSH not required for localhost - use local Docker client");
        }
    ...
    {
        // Localhost doesn't need SSH file transfer
        if (IsLocalhostServer(server))
        {
            _logger.LogDebug("Skipping SSH upload for localhost server {ServerName} - files are already local", server.Name);
            return true;
        }
    ...
    public async Task<bool> DownloadFileAsync(Server server, string remotePath, string localPath, CancellationToken cancellationToken = default)
    {
        ...
        if (IsLocalhostServer(server))
        {
            _logger.LogDebug("Skipping SSH download for localhost server {ServerName} - files are already local", server.Name);
            return true;
        }
    ...
    {
        // This should never be called for localhost
        if (IsLocalhostServer(server))
        {
            throw new InvalidOperationException($"SSH client should not be created for localhost server {server.Name}");
        }
