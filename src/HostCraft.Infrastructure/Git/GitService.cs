...
public class GitService : IGitService
{
    ...
    public async Task<string> CloneApplicationRepositoryAsync(Application application, string? commitSha = null)
    {
        ...
        // Ensure GitOwner and GitRepoName are populated (fallback for existing apps)
        if (string.IsNullOrEmpty(application.GitOwner) || string.IsNullOrEmpty(application.GitRepoName))
        {
            var parts = application.GitRepository.Split('/', StringSplitOptions.RemoveEmptyEntries);
            if (parts.Length >= 2)
            {
                application.GitOwner = parts[0];
                application.GitRepoName = parts[1];
            }
            else
            {
                throw new InvalidOperationException($"Invalid repository format: {application.GitRepository}. Expected 'owner/repo' format.");
            }
        }
        ...
        try
        {
            ...
            // Set GIT_LFS_SKIP_SMUDGE if LFS is disabled
            var envVars = new Dictionary<string, string>();
            if (!application.EnableGitLfs)
            {
                envVars["GIT_LFS_SKIP_SMUDGE"] = "1";
            }

            _logger.LogInformation(
                "Cloning repository {Owner}/{Repo} branch {Branch} to {Path} (LFS: {LfsEnabled}, Submodules: {SubmodulesEnabled})",
                application.GitOwner,
                application.GitRepoName,
                application.GitBranch,
                targetPath,
                application.EnableGitLfs,
                application.CloneSubmodules);

            await RunGitCommandAsync(args, _tempPath, envVars: envVars);
            ...
            if (application.EnableGitLfs)
            {
                try
                {
                    _logger.LogInformation("Pulling Git LFS files...");
                    await RunGitCommandAsync("lfs pull", targetPath);
                }
                catch (Exception lfsEx)
                {
                    // LFS might not be installed or repo might not use LFS - log but don't fail
                    _logger.LogWarning(lfsEx, "Git LFS pull failed (LFS may not be used by this repo)");
                }
            }
