...
public class AuthService : IAuthService
{
    ...
    private int _tokenExpirationMinutes;
    ...
    // Flag to track if JWT settings have been loaded
    private bool _jwtSettingsLoaded = false;
    private readonly SemaphoreSlim _jwtSettingsLock = new(1, 1);
    ...
    public AuthService(
        ...
        IJwtSettingsService jwtSettingsService,
        ILogger<AuthService> logger)
    {
        ...
        // Load non-JWT settings from configuration
    ...
    /// <summary>
    /// Ensures JWT settings are loaded from the database.
    /// </summary>
    private async Task EnsureJwtSettingsLoadedAsync()
    {
        if (_jwtSettingsLoaded) return;

        await _jwtSettingsLock.WaitAsync();
        try
        {
            if (_jwtSettingsLoaded) return;

            var settings = await _jwtSettingsService.GetJwtSettingsAsync();
            _jwtSecret = settings.Secret;
            _jwtIssuer = settings.Issuer;
            _jwtAudience = settings.Audience;
            _tokenExpirationMinutes = settings.ExpirationMinutes;
            _jwtSettingsLoaded = true;

            _logger.LogDebug("JWT settings loaded from database");
        }
        finally
        {
            _jwtSettingsLock.Release();
        }
    }
    ...
    public async Task<AuthResult> LoginAsync(string email, string password, string? ipAddress = null, string? userAgent = null)
    {
        try
        {
            // Ensure JWT settings are loaded before any token operations
            await EnsureJwtSettingsLoadedAsync();
    ...
    public async Task<bool> HasAnyUsersAsync()
    {
        try
        {
            ...
            return await _userRepository.AnyAdminsAsync();
        }
        catch (Exception ex)
        {
    ...
    {
        return await _auditLogRepository.GetPagedAsync(page, pageSize);
    }
    ...
    public async Task<AuthResult> RefreshTokenAsync(string refreshToken, string? ipAddress = null)
    {
        try
        {
            ...
            await _userRepository.UpdateAsync(storedToken.User!);
    ...
    {
        // JWT settings must be loaded before calling this method
        if (string.IsNullOrEmpty(_jwtSecret))
        {
            throw new InvalidOperationException("JWT settings not loaded. Call EnsureJwtSettingsLoadedAsync first.");
        }
